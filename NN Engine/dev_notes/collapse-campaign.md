...
## 2026-07-03 — SEARCH campaign (adopt SF11's missing techniques) — 3 levers screened, none shipped yet
Post-capg, re-aimed at the single-thread node-efficiency gap (SF11 beats us equal-depth ~23× fewer nodes; SMP off-target). Baseline WAC 260 / 62,056,876 / STS 51.9%. All work gated-off, byte-id; nothing shipped/regressed. **Gate rule:** we BEAT SF18 on WAC@d10 → tactics = STRENGTH, positional = weakness → gate at EQUAL-TIME (`wac_timed_depth`/`sts_timed_depth`); fixed-depth WAC = loose check; **STS is the axis that matters.**
- **seecap — SEE-prune LOSING captures** (`SEE_PRUNE_CAPTURES`/`_MARGIN`, both helpers, decoupled). margin=1000: fixed −14.8% nodes, equal-time +0.34ply/WAC-timed 259, **lightning SPRT ≈ WASH (~+17 Elo @249g)**. Lesson: cutting CAPTURE search spends our tactical over-provisioning → ~0. Deprioritize capture-pruning (seecap/ProbCut). Gated-off.
- **ROOT PRE-SEARCH is LOAD-BEARING, not removable.** `reorder_legal_moves` runs a full d-1 pre-pass every iteration (~+26% nodes) that GENERATES the 2nd-level move lists the real `minimizer` consumes + warms the TT. Two gated knobs: `ROOT_PRESEARCH_REDUCTION` (byte-id@1, depth>=2 guard; sweep is CONFOUNDED — shallow scores not a clean fallback, WAC craters −20@r2) and `ENABLE_ROOT_PRESEARCH` hard-off (reuse prev-iter real 2nd-level data; invariants trace-verified vs CRASH_INVESTIGATION_PLAYBOOK, NO crash) = STRICTLY WORSE **77 solves / +32% NODES** — removing it INCREASES nodes (its warmup/ordering more than pays for itself). ⇒ pre-search STAYS.
- **TT best-move ordering BUILT + parked** (`ENABLE_TT_MOVE`, `g_ttMoveTable` TT-sized; write at min/max beta-cutoff, promote-front in `buildMoveListFromReordered` via `promoteMoveToFront`; TTEntry has no move field so this fills the gap). Byte-id. Alone: 254/−3.4% nodes. **Does NOT rescue pre-search removal** (hard-off+TT still 81/81M → pre-search value = deep TT-warmup, not the hash move). Equal-time MARGINAL: WAC-timed +5 but **STS-timed −2.7%** (wrong trade — helps our strength, hurts our weakness). PARKED.
- **NEXT (worklist):** T2 null-move R under-scaling (`NULLMOVE_PROGRESSIVE` — flat R≈2 across d10-13, null-only no LMR coupling); T3 SPSA micro-cluster (promote `FUTILITY_MARGINS` constexpr→knobs, name razoring magic-consts, then SPSA the cluster via `spsa.py`); T4 history-decay divergences (moveFrequency ÷4 vs ÷2, `*16` cadence, 4-16× bonus swing). METHOD: accumulate STABLE tactic-safe cuts → bundle → ONE equal-time SPRT (a lone ~3-15% cut is below the SPRT floor). NOTE: combo1 aggression (LMP5/null2/HISTORY_LMR_SCALE2) is ALREADY in the current defaults.

## 2026-07-02b — NEG RESULTS + parked infra: flat positional terms dead 3× ; Open Files at a non-tension-separable frontier
Full session hunting the next eval win after capg. All-negative but high-value knowledge (nothing shipped, nothing regressed — engine byte-id at defaults 260/62,056,876). Everything below is **gated-off, byte-id, committed for reuse.**
- **Scalar/mobility conditioning hunt = NEGATIVE.** `pvb_delta` (added `pvb` dispatcher sub) on the weak themes: `mobility`/`pawn_struct`/`pawn_majority` read flat ~0 delta; `outpost` only a weak helper (can't overcome `pieces`). The remaining STS static gaps are (a) SEARCH/horizon (best move = a sac our static correctly under-reads) and (b) HARD positional knowledge (space/maneuvering).
- **`space` term BUILT + DEAD-as-flat.** New gated term: pawn+knight rank-weighted enemy-half control (`SPACE_MAG`/`SPACE_KNIGHT_MAG`/`SPACE_PHASE_MAX`, cpp_bitboard.cpp after outpost). Phase-gate fixed endgame poisoning; knight component = noise (split off, default 0); pawn-only = real correlational signal (Open Files +94, 7th +48) but **net-flat + NON-MONOTONIC in magnitude** = noise into search. 3rd confirmation flat additive terms don't ship.
- **Foundation maps are LOAD-BEARING, not noise.** `SCALE_CENTRAL=0` = −234 STS (central is NOT a redundant replica of the attack layer, despite an Explore agent's claim). ⇒ "redefine the situation-blind maps to de-noise" is really the hard NNUE-precision problem, not easy cleanup.
- **Open Files/7th under-served but at a real PARETO FRONTIER.** Three flat levers (add space, remove central, boost rook knobs) all give the IDENTICAL swing: lift Open Files/7th/Advancement, drop Recapturing/King Activity — net flat/negative.
- **Tension-conditioned rook-file term BUILT + DISCONFIRMED.** Reuses `g_capg_tension` (capg template): boost rook open-file/7th/connected in quiet positions via `g_rook_file_bonus` accumulator + post-capg `rook_tension_scale()` rescale (`ENABLE_ROOK_TENSION_COND`/`ROOK_COND_TENSION_LO/HI`/`ROOK_COND_QUIET_SCALE`; midgame-only by construction). HI=3 = −341; HI=1 (tension-0-only) = −168 AND the Open Files gain VANISHED → **the file gains live in the SAME tension regime as recaptures, so tension can't separate them.** Frontier is tension-aligned in NAME only.
- **⇒ Clean/cheap eval levers for these positional themes are EXHAUSTED.** The only positive-netting static lever this campaign = conditioned SUBTRACTION (capg, shipped). Crossing this frontier needs a separator detector we don't have (maybe NNUE-territory). PAUSED for discussion. Full detail: [[eval-theme-diagnostic-loop]], [[eval-accuracy-payoff-is-pruning]], [[coach-api-positional-detectors]] (external ref for future term geometry).

## 2026-07-02 — SHIPPED: capg tension-conditioning + LMP_BASE=1 (`236b1f0`, first position-conditional eval)
The bundle campaign (mobility/scalars) concluded **dead**: flat SF-parity terms are neutral in games (search masks the accuracy at our depth), cost NPS, and make aggressive pruning worse — `bundle ≈ capg70 solo ≈ +24` (marginal of the 4 new terms over capg70 ≈ 0). Lesson locked: **eval accuracy ≠ strength; the levers are (a) SUBTRACT decision-flipping over-reads and (b) RAISE THE PRUNING CEILING** (a cleaner eval unlocks aggression — capg70 flipped `LMP_BASE=1` from a −149-STS crater on the default eval to +39 STS / −4% nodes). NNUE is premature (need a stronger teacher-eval + data; keep building HCE craft — user's call).

Built the **STS-failure × SF11-breakdown diagnostic loop** (`sts_full` per-theme scoreboard, `sts_gap`/`diagnostics/sts_sf11_gap.py` our-vs-SF11 per-term on a theme's failures, `diagnostics/pvb_delta.py` played-vs-best per-term DELTA = the decisive culprit tool — aggregate signed means are noise). It found capture_gains is **context-dependent**: bench data (flat capg 0/70/100 per-theme) shows capture/tactical themes want capg HIGH (Recapturing/Advancement/Pawn Play), quiet/trading themes want it LOW (Simplification/AT/Center Control), WAC wants MAX — a flat scalar (capg70) is the least-bad compromise.

**Fix = capg × f(tactical tension).** Detector `g_capg_tension` = # of viable SEE≥0 captures, computed FREE inside `approximate_capture_gains`; ramp `capg_conditioned_scale()` slides capg from `CAPG_LO_SCALE` (quiet, tension≤LO) to `CAPG_HI_SCALE` (tactical, tension≥HI). Shipped ramp **condE** (LO0/HI3, 10→100) + `LMP_BASE=1`: vs default base **WAC 260 (+8), STS 51.9% (+1.1), nodes −11.5%, benoni a4b5**. **Lightning SPRT ~+15 Elo (555-495-302, no-regression / 1352 games)** — SELF-LAUNCHED via native-ELF SF (interop finally resolved; `overnight_runner.sh` SF var → `stockfish_18_linux/…avx2`). Gated: `ENABLE_CAPG_COND=false` reverts byte-id (252/70,150,573, verified both ways). Defaults flipped in `search_engine.h`; new baseline = 260/62,056,876/STS 51.9%.

**NEXT** (decided w/ user): CONDITIONED mobility/scalars, **diagnostic-FIRST** (find their home context via pvb_delta on weak themes BEFORE building — flat versions are proven-dead, don't re-litigate); prefer the scalars (structural/timeless) over mobility (activity the search resolves); gate on the scoreboard WITH capg-cond on. See [[capg-tension-conditioning]], [[eval-theme-diagnostic-loop]], [[eval-accuracy-payoff-is-pruning]].
