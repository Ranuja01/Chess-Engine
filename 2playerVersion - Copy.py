...
class Layout(tk.Tk):
    ...
    def get_location(self, event, i, j):
        ...
        if (not(self.isPromotion)):
            ...
            if (self.move):
                ...
                if (self.numMove % 2 == 1):
                    if(self.blackSideEnPasent and self.pieceToBeMoved.piece == "Pawn" and i == self.blackSideEnPasentPawnxLocation and isCapture):
                        print("DDDDWE")
                        curItem = self.boardPieces[self.blackSideEnPasentPawnxLocation - 1][4]
                        
                else:
                    if(self.whiteSideEnPasent and self.pieceToBeMoved.piece == "Pawn" and i == self.whiteSideEnPasentPawnxLocation and isCapture):
                ...
                if(isLegal):
                    ...
                    if(isLegal): 
                        ...
                        if(self.isCheck): 
                            ...
                            pieces = self.canvas.create_image(45 + (kingPiece.xLocation-1)*90,45 + (8-kingPiece.yLocation)*90,image = piece, anchor = 'center')
                            self.canvas.tag_bind(pieces,"<Button-1>", lambda e, x = kingPiece.xLocation, y=kingPiece.yLocation: self.get_location(e,x,y))
                        print("White: " + str(self.evaluateBoard("White")))
                        print("Black: " + str(self.evaluateBoard("Black")))
                        self.numMove += 1
                        board.update()
                
                        if (not(self.isComputerMove)):
                            currentItem,pieceToBeMoved,val = self.maximizer(0,3,"Black","Black")   
    ...
    def maximizer(self,curDepth,depthLimit,colour,evalColour):
        """
        if(self.count % 10 == 0):
            print(self.count)
        self.count += 1
                    ...
                    for item in moves:
                        ...
                        if(isLegal):
                            ...
                            if(not(self.isInCheck(colour))):
                                ...
                                if(self.evaluateBoard("Black") - self.evaluateBoard("White") > highestScore):
                                    score = self.minimizer(curDepth + 1,depthLimit,colour,evalColour)
        ...
        if (curDepth == 0):       
            return curItem,pieceToBeMoved,highestScore
        else:
            return highestScore
        
        
    def minimizer(self,curDepth,depthLimit,colour,evalColour):
        if (curDepth >= depthLimit):
            if(evalColour == "Black"):
                oppositeColour = "White"
            else:
                    ...
                    for item in moves:
                        ...
                        if(isLegal):
                            ...
                            if(not(self.isInCheck(colour))):
                                ...
                                score = self.maximizer(curDepth + 1,depthLimit,colour,evalColour)
